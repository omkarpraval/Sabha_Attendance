...
import { Search, UserCheck, CheckCircle2, XCircle, Clock, Eye, QrCode, ShieldAlert, Award, Calendar, ChevronDown } from 'lucide-react';
...
export default function KaryakarPortal({ user, onUserUpdated }) {
  ...
  const loadEventsAndUsers = async () => {
    try {
      setLoading(true);
      const [evList, memberList, pendingList] = await Promise.all([
        apiFetch('/events'),
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
      ...
      {/* Event Selection & Karyakar Header Banner */}
      <div className="bg-white rounded-2xl p-6 warm-shadow border border-[#EFE7DA] space-y-4">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-[#EFE7DA] pb-4">
          <div>
            <div className="flex items-center gap-2 text-[#E8A33D] text-xs font-bold uppercase tracking-wider mb-1">
              ...
              <span>Karyakar Portal • Attendance Management</span>
            </div>
            <h2 className="font-serif-accent text-2xl md:text-3xl font-bold text-[#8B3A3A]">
              Volunteer Attendance Control
            </h2>
          </div>
        ...
        <div className="bg-[#FDFBF7] p-4 rounded-xl border border-[#EFE7DA] flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="w-full md:w-2/3">
            ...
            <div className="relative">
              <select
                ...
                className="w-full p-2.5 pr-8 rounded-xl border border-[#EFE7DA] bg-white text-xs font-semibold text-[#3A322C] focus:outline-none focus:border-[#8B3A3A] cursor-pointer appearance-none"
              >
                {events.map((ev) => (
          ...
          <div className="bg-white px-5 py-2.5 rounded-xl border border-[#EFE7DA] text-center w-full md:w-auto">
      ...
      {/* Pending User Approval Queue (Visibility ONLY for Karyakar) */}
      {pendingUsers.length > 0 && (
        <div className="bg-[#FFF2CC]/60 rounded-2xl p-4 border border-[#D9B166] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#D9B166] text-white flex items-center justify-center shrink-0">
              <Clock className="w-5 h-5" />
            </div>
            <div>
              <h4 className="font-semibold text-sm text-[#3A322C]">
                {pendingUsers.length} Pending New User Signup Request(s)
              </h4>
              <p className="text-xs text-[#3A322C]/70">
                Awaiting final review & approval by Admin (Karyakar View Only)
              </p>
            </div>
          </div>

          <div className="text-xs font-bold text-[#D9B166] bg-white px-3 py-1.5 rounded-lg border border-[#D9B166]/40 self-end sm:self-auto">
            View Queue
          </div>
        </div>
      )}

      {/* Searchable Member Directory & Manual Attendance Marking */}
      <div className="bg-white rounded-2xl p-6 warm-shadow border border-[#EFE7DA] space-y-4">
